...
def drop_path(x, drop_prob: float = 0., training: bool = False):
    ...
    Taken directly from https://github.com/rwightman/pytorch-image-models/blob/a6e8598aaf90261402f3e9e9a3f12eac81356e9d/timm/models/layers/drop.py
    so installing timm is optional
    """
...
class PoolingTransformer(GeneralNet):
    def __init__(self, 
                 image_size: int = 32, 
                 patch_size: int = 4, 
                 stride: int = 1, 
                 base_dims: List[int] = [32, 32], 
                 depth: List[int] = [10, 2], 
                 heads: List[int] = [6, 12],
                 mlp_ratio: float = 4.0, 
                 num_classes: int = 10, 
        ...
        super(PoolingTransformer, self).__init__(num_classes, in_chans, model_path, seed)
    ...
    def forward(self, x):
        batch_size = x.shape[0] # ART compatibility

        cls_features = self.forward_features(x)
        output = self.head(cls_features)

        output = torch.reshape(output, (batch_size, self.n_classes)) # ART compatibility

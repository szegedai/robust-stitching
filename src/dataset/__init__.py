...
def get_datasets(str_dataset):
    ...
    if str_dataset == 'fashion' or str_dataset == 'fashionmnist':
        trans = transformer['fashion']
    ...
    elif str_dataset == 'svhn' :
        trans = transformer['svhn']
        return create_svhn_datasets(trans)
    elif str_dataset == 'imagenet' :
        trans = transformer['imagenet']
        return create_imagenet_datasets(trans)
...
def create_svhn_datasets(trans):
    root = get_root('pytorch')
    train_trans = T.Compose(trans[0])
    val_trans = T.Compose(trans[1])

    train = torchvision.datasets.SVHN(root, split="train", download=True, transform=train_trans)
    val = torchvision.datasets.SVHN(root, split="test", download=True, transform=val_trans)
...
def create_imagenet_datasets(trans):
    root = get_root("imagenet")
    train_trans = T.Compose(trans[0])
    val_trans = T.Compose(trans[1])

    train = torchvision.datasets.ImageFolder(f"{root}/training_data", transform=train_trans)
    val = torchvision.datasets.ImageFolder(f"{root}/validation_data", transform=val_trans)
